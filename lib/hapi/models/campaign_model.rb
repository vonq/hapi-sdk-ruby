...
module Hapi
  ...
  class CampaignModel < BaseModel
    ...
    # @return [List of OrderedProductsGetElementModel]
    attr_accessor :ordered_products_specs

    # This part contains information of the ordered products.
    # @return [List of CampaignPostingModel]
    ...
    def initialize(company_id = nil,
      ...
      @postings = postings unless postings == SKIP
    ...
    def self.from_hash(hash)
      ...
      unless hash['orderedProductsSpecs'].nil?
        ...
        hash['orderedProductsSpecs'].each do |structure|
          ordered_products_specs << (OrderedProductsGetElementModel.from_hash(structure) if structure)
        end
      end
      ...
      # Parameter is an array, so we need to iterate through it
      postings = nil
      unless hash['postings'].nil?
        postings = []
        hash['postings'].each do |structure|
          postings << (CampaignPostingModel.from_hash(structure) if structure)
        end
      end

      postings = SKIP unless hash.key?('postings')

      # Clean out expected properties from Hash.
      names.each_value { |k| hash.delete(k) }

      # Create object from extracted values.
      CampaignModel.new(company_id,
                        recruiter_info,
                        posting_details,
                        target_group,
                        ordered_products,
